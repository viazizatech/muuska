...
<?php 
    ...
    $hasSubValuesClass = '';
    if ($hasSubValues) {
        $newAttributes['data-ktmenu-submenu-toggle'] = 'hover';
        $hasSubValuesClass = ' kt-menu__item--submenu';
    }
    $defaultLink = $item->getDefaultLink($globalConfig, $callerConfig);
?>
<?php echo $item->drawStartTag('li', $globalConfig, $callerConfig, 'kt-menu__item kt-menu__item--rel'.$hasSubValuesClass, null, $newAttributes)?>
...
<?php echo $item->drawItems($globalConfig, $callerConfig, '<div class="kt-menu__submenu"><span class="kt-menu__arrow"></span><ul class="kt-menu__subnav">', '</ul></div>')?>
