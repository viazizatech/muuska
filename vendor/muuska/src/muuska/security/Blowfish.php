...
define('UNPACK_NATIVE', 1);
define('UNPACK_MODIFIED', 2);

class Blowfish extends Crypt_Blowfish
{
    /**
     * {@inheritDoc}
     * @see \muuska\security\Crypt_Blowfish::encrypt()
     */
    ...
    /**
     * {@inheritDoc}
     * @see \muuska\security\Crypt_Blowfish::decrypt()
     */
...
class Crypt_Blowfish
{
    ...
    protected $_unpackMode = UNPACK_NATIVE;
    ...
    public function __construct($key, $iv)
    {
        ...
        if (PHP_VERSION_ID == '50201' || PHP_VERSION_ID == '50206') {
            $this->_unpackMode = UNPACK_MODIFIED;
        }
    ...
    public function encrypt($plainText)
    {
        ...
        for ($i = 0; $i < $len; $i += 8) {
            list(, $Xl, $Xr) = ($this->_unpackMode == UNPACK_NATIVE ? unpack('N2', substr($plainText, $i, 8)) : $this->myUnpackN2(substr($plainText, $i, 8)));
    ...
    public function decrypt($cipherText)
    {
        ...
        for ($i = 0; $i < $len; $i += 8) {
            list(, $Xl, $Xr) = ($this->_unpackMode == UNPACK_NATIVE ? unpack('N2', substr($cipherText, $i, 8)) : $this->myUnpackN2(substr($cipherText, $i, 8)));
